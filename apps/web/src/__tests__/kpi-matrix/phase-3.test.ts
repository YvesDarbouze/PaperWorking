...
describe('Phase 3: Debt Sizing & Capital Stack — Verification Units (KPIs 19–26)', () => {
  let goldenProject: ProjectSummary;
  let baseMetrics: ProjectMetricsResult;
  const phase3Kpis = getKpisByPhase(3);
  ...
  test('Phase 3 contains exactly 8 KPIs', () => {
    expect(phase3Kpis).toHaveLength(8);
  });

  describe.each(phase3Kpis.map((kpi) => [kpi.number, kpi.id, kpi.name, kpi]))(
    'KPI #%i: %s (%s)',
    (num, id, name, kpi) => {
      test('CHECK A — WIRING: responds dynamically to input perturbation or honestly reports blocked', async () => {

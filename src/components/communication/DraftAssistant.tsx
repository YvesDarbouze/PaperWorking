...
interface DraftAssistantProps {
  deal: Project;
  /** Lifted state — persists through open/close cycles */
  draftContext: DraftContext;
  onDraftContextChange: (ctx: DraftContext) => void;
  onClose: () => void;
...
export default function DraftAssistant({
  ...
  const setAudience = (audience: 'investors' | 'contractors') =>
    onDraftContextChange({ ...draftContext, audience });

  const clearDraft = () => onDraftContextChange({ audience: draftContext.audience, draft: '' });
  ...
  const { audience, draft } = draftContext;
  ...
  return (
    ...
    >
      <header className="p-10 border-b border-border-accent flex justify-between items-center bg-pw-black">
        ...
        <button
          id="draft-assistant-close"
          onClick={onClose}
          className="p-2 hover:bg-pw-border/20 transition-all text-pw-white/50 hover:text-pw-white"
          ...
          <div className="grid grid-cols-1 border-t border-l border-border-accent">
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <span className={`text-xs font-black uppercase tracking-[0.2em] ${audience === 'investors' ? 'text-pw-white' : 'text-text-primary'}`}>
                  Equity Stakeholders
                </span>
              </div>
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <span className={`text-xs font-black uppercase tracking-[0.2em] ${audience === 'contractors' ? 'text-pw-white' : 'text-text-primary'}`}>
                  Operational Labor
                </span>
              </div>
          ...
          {draft && (
            ...
            >
              <div className="flex justify-between items-center">
                ...
                <button
                  id="draft-clear-btn"
                  className="text-xs font-black text-pw-accent uppercase tracking-widest hover:underline"
                  onClick={clearDraft}
                >
                  PURGE INDEX
                </button>

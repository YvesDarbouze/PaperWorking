/** Billing settings visual tokens — subscription & payment ops desk. */

import type { CSSProperties } from 'react';

export function billingTokens(isDark: boolean) {
  return {
...
export type BillingTokens = ReturnType<typeof billingTokens>;

export function panelStyle(t: BillingTokens): CSSProperties {
  return {
...
export function statusStyle(t: BillingTokens, status: string): { bg: string; fg: string } {
  switch (status) {
    case 'active':
    case 'trialing':
    case 'paid':
      return { bg: t.successMuted, fg: t.success };
    case 'past_due':
    case 'open':
      return { bg: t.warnMuted, fg: t.warn };
    case 'canceled':
      return { bg: t.alertMuted, fg: t.alert };
    default:
      return { bg: t.surfaceMuted, fg: t.muted };
  }
}

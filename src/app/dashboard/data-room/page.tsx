...
export default function DataRoomPage() {
  ...
  const handleExportPDF = useCallback(async () => {
    const selectedMetricsList = projectMetrics.filter((m) => selectedProjectsForReport.includes(m.id));
    if (selectedMetricsList.length === 0) {
      toast.error("No project data selected to export.");
      return;
    }
    ...
    try {
      ...
      selectedMetricsList.forEach((metrics) => {
        ...
        totalCapitalRaised += data.capitalRaised;
        totalUnits += (f.numberOfUnits ?? 1) * factor;
        totalOccupiedUnits += (f.occupiedUnits ?? (f.numberOfUnits ?? 1)) * factor;

        weightedCoC += data.coc * data.capitalRaised;
        weightedAppreciation += data.appreciation * val;
      });
      ...
      const summaryBody = [
        ["Selected Properties", selectedMetricsList.length.toString()],
        ["Total Portfolio Value", fmtUSD(totalValue)],
        ["Total Equity", fmtUSD(totalEquity)],
        ["Total Capital Raised", fmtUSD(totalCapitalRaised)],
        ["Net Operating Income (NOI)", fmtUSD(totalNOI)],
        ["Annual Cash Flow", fmtUSD(totalCashFlow)],
        ["Portfolio Cap Rate", fmtPct(capRate)],
        ["Cash-on-Cash Return (CoC)", fmtPct(coc)],
        ["Portfolio Occupancy Rate", fmtPct(occupancy)],
      ];
      ...
      const tableBody = selectedMetricsList.map((proj) => {
        const data = scope === "Property" ? proj.asset : proj.investor;
        return [
          proj.propertyName,
  ...
  }, [selectedProjectsForReport, projectMetrics, activeProjects, scope]);

...
export default function FocusedWorkflowLayout({
  ...
  return (
    <div className="fixed inset-0 z-[200] flex flex-col bg-pw-white text-pw-black overflow-hidden animate-in fade-in duration-300">
      
      {/* Structural Header */}
      <header className="h-20 border-b border-pw-border flex items-center justify-between px-10 bg-pw-white shrink-0">
        <div className="flex items-center gap-8">
          ...
          <div>
            ...
            {subtitle && <p className="text-xs text-pw-muted font-bold uppercase tracking-wider mt-1">{subtitle}</p>}
          </div>
        </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <aside className="w-[320px] border-r border-pw-border bg-pw-bg flex flex-col p-12 shrink-0 overflow-y-auto">
          <div className="mb-12">
            <p className="text-xs font-black text-pw-muted uppercase tracking-[0.4em] mb-4">WORKFLOW_PROGRESS</p>
              ...
              return (
                <div key={step.id} className="flex gap-6 relative">
                  ...
                  <div className={`w-7 h-7 shrink-0 flex items-center justify-center font-mono text-xs font-black border transition-all ${
                    ...
                    'bg-pw-white text-pw-muted border-pw-border'
                  }`}>
                  ...
                  <div className="pt-1">
                    <p className={`text-xs font-black uppercase tracking-widest ${
                      isActive ? 'text-pw-black' : 
                      isComplete ? 'text-pw-black/60' : 
                      'text-pw-muted'
                    }`}>
                      {step.label}
                    </p>
          ...
          <div className="mt-auto pt-12 border-t border-pw-border">
            <div className="flex items-center gap-3 grayscale opacity-30">
              <div className="w-2 h-2 rounded-full bg-pw-accent animate-pulse" />
              <span className="text-xs font-black uppercase tracking-widest text-pw-black">Ledger_Sync_Active</span>
            </div>
          </div>
        </aside>

        {/* Main Content Area */}
        <main className="flex-1 bg-pw-white overflow-y-auto p-20 flex flex-col">
          <div className="max-w-3xl w-full mx-auto flex-1">
            {children}
          </div>

          {/* Persistent Footer Actions (Hierarchy Enrollment) */}
          <div className="max-w-3xl w-full mx-auto mt-20 pt-10 border-t border-pw-border flex justify-between items-center">
            {onBack ? (
              <button 
                onClick={onBack}
                className="flex items-center gap-3 text-xs font-black text-pw-muted hover:text-pw-black uppercase tracking-[0.2em] transition-all"
              >
            ...
            {onNext && (
              <button 
                ...
                className={`
                  ...
                  ${isNextDisabled 
                    ? 'bg-pw-bg text-pw-muted border-pw-border cursor-not-allowed opacity-50' 

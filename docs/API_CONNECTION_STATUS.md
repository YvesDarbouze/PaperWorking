...
| GET | `/api/support/faq` | SupportCenter (search/browse FAQs) |
| POST/PATCH | `/api/support/faq` | Restricted to verified Firebase `admin` custom claim (seeding via `npm run db:seed:support`) |
| GET | `/api/support/glossary` | SupportCenter (alphabetical glossary) |
| POST/PATCH | `/api/support/glossary` | Restricted to verified Firebase `admin` custom claim (seeding via `npm run db:seed:support`) |
| POST | `/api/support/pepper` | SupportCenter (grounded Pepper AI chatbot with output guard) |
| POST | `/api/support/submissions` | SupportCenter (feature requests & suggestions) |
| POST | `/api/support/callback` | SupportCenter (investor call-back requests) |
| GET/POST | `/api/projects/[id]/documents` | FundWorkspaceView (Document Vault upload & list) |
| GET/DELETE | `/api/projects/[id]/documents/[docId]` | Document Vault (download, delete with IRS 3-year lock, & `?mode=signedUrl` object-level proxy $\le 15$ min) |
...
| `deal-calculator` | 1 | Renamed from deal-analyzer [historical changelog]; canonical route /api/deal-calculator/property-lookup with historical alias |
...
| GET | `/api/cron/jobs-sweep` | `handleJobSweep` |
...
| POST | `/api/jobs/sweep` | `handleJobSweep` |
...
## E. Security Posture, Storage Proxying & Rules Audit (Review C2.3 / P-03 Closure)

1. **Default-Deny Everywhere:**
   - `firestore.rules`: Unconditional catch-all `match /{document=**} { allow read, write: if false; }`. Explicit per-collection policies implemented (`support_faq`, `support_glossary` public-read / write-false; `feedback`, `callbacks` client-read:false / write:false [Admin SDK only]; `users/{uid}` owner-only; `deals` authenticated-read / write-false).
   - `storage.rules`: Unconditional catch-all `match /{allPaths=**} { allow read, write: if false; }`.
2. **P-03 Vulnerability Remediated:**
   - Previous open read on `/deals/{dealId}/documents/*` completely revoked (`allow read, write: if false;`).
   - Sensitive project and deal files proxy strictly through the server-side API (`/api/projects/[id]/documents/[docId]?mode=signedUrl`) requiring object-level membership verification (`requireWebOrgRole`).
   - All generated signed download URLs are cryptographically capped at $\le 15$ minutes (900 seconds).
3. **Tenant-Scoped Storage & Quarantine Buffer:**
   - Direct tenant uploads at `/orgs/{orgId}/**` enforce custom claim membership (`request.auth.token.organizationId == orgId || request.auth.token.orgId == orgId`) and max 50MB payload limits.
   - Per-user staging upload quarantine areas at `/temp/{uid}/**` and `/users/{uid}/temp/**` enforce `isOwner(uid)` and max 25MB limits.
4. **CI-Only Rules Deployment:**
   - Security rules deployment is automated via CI only; ad-hoc developer workstation deployments are blocked. Detailed in [`docs/FIREBASE_RULES_DEPLOYMENT.md`](./FIREBASE_RULES_DEPLOYMENT.md).

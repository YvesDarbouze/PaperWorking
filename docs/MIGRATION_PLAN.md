...
| 4v | Plaid liabilities/pause + notifications test + deal-calculator (formerly deal-analyzer [historical changelog]) + LOI + zoning + packages share + projects list | ✅ plaid/liabilities, exchange-public-token, connections DELETE/pause, notifications/test, deal-calculator/property-lookup (formerly deal-analyzer/property-lookup [historical changelog; alias preserved]), loi/generate, zoning-scan, packages/share, projects GET list |
...
## 10. Post-Migration Hardening & Operational Guarantees (September 2026)

### 10.1 Silent Degradation Elimination (Reviews C1.1, C1.2)
- **Boot Assertions:** `assertBootEnvironment()` enforced on cold boot in `apps/api` and `apps/web` (Next.js `instrumentation.ts`).
- **Fail-Fast Production Invariants:** When `NODE_ENV=production`, `DATABASE_URL` is strictly required; `DATA_MODE` must be `postgres` (`disk` fallback rejected); `SENDGRID_API_KEY` is required for transactional emails.
- **Explicit Adapter Responses:** SendGrid adapter returns typed error `{ success: false, status: 503, simulated: false }` when credentials are absent; disk fallback for deal calculator snapshots is strictly disallowed unless `ALLOW_DISK_FALLBACK=true` in development.

### 10.2 Durable Jobs Layer & Alert Runner (Reviews C1.3, C1.4, H-21)
- **Durable Storage & Atomic Sweeps:** `jobs` and `alert_deliveries` tables added to Prisma schema (`20260911_durable_jobs_and_alert_runner`). Sweeps claim due jobs atomically using `FOR UPDATE SKIP LOCKED` on Neon/PostgreSQL, preventing concurrent worker races.
- **External Scheduler Integration:** Cloud Scheduler / Vercel Cron Pro sweeps `POST /api/jobs/sweep` or `GET /api/cron/jobs-sweep` at $\le 15$-minute intervals with `CRON_SECRET` / Bearer token authentication.
- **Downtime Recovery Guarantee:** Sweeper automatically claims all overdue pending jobs (`due_at <= NOW()`), backfilling missed alerts seamlessly upon recovery (verified via 26-hour outage simulation test).
- **Delivery Guarantees & Dead-Letter Threshold:**
  - Per-alert idempotent delivery tracking (`alert_id` + `channel`) preventing double-sends.
  - Exponential backoff schedule (`1m`, `5m`, `15m`, `1h`) for transient delivery failures.
  - Emergency dead-letter cutoff at $T-6\text{h}$ before contractual deadline: immediately surfaces critical in-app banner alert and fires ops notification before earnest money hard dates expire.
  - Detailed delivery audit trail persisted on `AlertDelivery`.
- **Contractual Deadline Semantics & Timezone Integrity:**
  - IANA timezone (`ianaTimezone`) derived deterministically from deal property address and stored on `Project` and `Deal`.
  - Supports `calendar_days`, `business_days` (excluding weekends), and `exact_hours` calculation.
  - Golden-tested wall-clock preservation (5:00 PM local) across Daylight Saving Time transitions (Spring Forward 23h, Fall Back 25h) and month-ends.
- **Observability:**
  - Runner sweep counters (`claimed`, `succeeded`, `failed`, `retried`, `dead_lettered`).
  - System health endpoint (`/api/health`) surfaces `alert_backlog_age_seconds`, `pending_jobs`, and `dead_letter_jobs`.

### 10.3 Support Knowledge Base Hardening & Poisoning Defense (Review C2.1, P-01)
- **Unauthenticated Write Surface Elimination:** `POST` and `PATCH` on `/api/support/faq` and `/api/support/glossary` are strictly gated behind Firebase ID token verification with custom claim `admin: true`. Unauthenticated requests receive 401 Unauthorized; non-admin users receive 403 Forbidden.
- **Server-Local Seeding Workflow:** Seeding and maintenance migrated to `scripts/seed-support-firestore.ts` (`npm run db:seed:support`), executing directly via Admin SDK with zero HTTP network exposure.
- **Content Security & Prompt Injection Defense:** Server-side validation via `content-filter.ts` enforces Zod schemas and strictly rejects/redacts email addresses (`@`), external URLs, script tags, and prompt injection patterns (400 Bad Request) before any document touches Firestore or memory storage.
- **Pepper AI Output Guard & Escalation:** Stream output guard redacts all internal emails (`hi@paperworking.co`, `support@paperworking.co`, and any generic email) and guarantees zero occurrences of the `@` symbol in Pepper's chunked output stream. System instructions strictly prohibit contact string output and mandate escalation exclusively via the on-page Support Form or Call Back Form.

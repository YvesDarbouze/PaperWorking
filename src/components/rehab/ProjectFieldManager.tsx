          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-5 mt-6">
            <div className="flex items-center justify-between mb-4">
               ...
               {rehabTasks.some((t: any) => t.escrowDrawRequested) && (
                 <span className="px-2 py-1 bg-green-100 text-green-700 text-[10px] font-bold rounded-full border border-green-200 hidden sm:block">
                   ACTIVE ESCROW DRAWS
                 </span>
               )}
            </div>
                ...
                <div key={task.id} className="relative flex items-center justify-between z-10 p-3 bg-white border rounded-xl shadow-sm hover:shadow-md transition">
                   ...
                   {task.status !== 'Complete' && (
                      <button 
                        ...
                        className="ml-2 text-[10px] px-3 py-1.5 bg-gray-900 text-white font-bold rounded hover:bg-gray-800 transition active:scale-95 shadow-sm"
                      >
                        MARK DONE
                      </button>
                   )}

/**
 * Centralized organization role normalization.
 * Does not redesign the schema — validates/normalizes free strings.
 */
...
export function isAllowedOrgRole(role: string | null | undefined): boolean {
  const n = normalizeOrgRole(role);
  const allowed = new Set([...ORG_MANAGE_NORMALIZED, ...ORG_NON_MANAGE_NORMALIZED]);

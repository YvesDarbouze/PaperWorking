          ...
          {!isSuccess ? (
            <div className="p-16">
              <header className="mb-12">
                <p className="text-xs font-black uppercase tracking-[0.4em] text-pw-subtle mb-3">Procurement Request</p>
                <h2 className="text-3xl font-black text-pw-black tracking-tighter uppercase mb-1">Transmit Briefing</h2>
                <p className="text-xs font-black text-pw-muted uppercase tracking-widest">Entity: {vendor.companyName}</p>
              </header>

              <div className="space-y-10">
                {/* Deal Selection */}
                <div>
                  <label className="block text-xs font-black uppercase tracking-widest text-pw-subtle mb-4">Target Portfolio Asset</label>
                  <div className="relative">
                    <select 
                      className="w-full px-6 py-4 bg-pw-dashboard border border-pw-border rounded-none text-xs font-bold uppercase tracking-widest appearance-none focus:outline-none focus:border-pw-black transition-all cursor-pointer"
                ...
                <div>
                  <div className="flex items-center gap-3 mb-6">
                    ...
                    <h3 className="text-xs font-black text-pw-black uppercase tracking-[0.2em]">Restricted Document Provisioning</h3>
                  </div>
                  <div className="border border-pw-border divide-y divide-pw-border bg-pw-dashboard">
                    <FolderCheckbox 
            ...
            <div className="p-24 text-center">
              ...
              <h3 className="text-3xl font-black text-pw-black mb-4 uppercase tracking-tighter">Transmission Confirmed</h3>
  ...
  return (
    <button 
      ...
      className={`w-full text-left p-6 transition-all flex items-start gap-6 ${
        checked ? 'bg-white' : 'bg-pw-dashboard/50 hover:bg-pw-dashboard'
      }`}
    >
      <div className={`mt-0.5 w-4 h-4 border flex items-center justify-center transition-all ${
        checked ? 'bg-pw-black border-pw-black' : 'bg-white border-pw-border'
      }`}>
        {checked && <div className="w-1.5 h-1.5 bg-white" />}
      </div>
      <div>
        <p className={`text-xs font-black uppercase tracking-widest ${checked ? 'text-pw-black' : 'text-pw-muted'}`}>{label}</p>
        <p className="text-xs font-bold text-pw-subtle mt-1 uppercase tracking-tight">{description}</p>
      </div>
    </button>
  );
}

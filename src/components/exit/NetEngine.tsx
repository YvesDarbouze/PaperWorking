    ...
    <div className="bg-pw-black border border-gray-800 rounded-2xl overflow-hidden shadow-2xl mt-8">
      <div className="p-6 border-b border-gray-800 flex items-center justify-between bg-black/40">
         <div className="flex items-center space-x-3">
           ...
           <div>
             ...
             <p className="text-xs text-gray-500 font-mono tracking-widest uppercase">The Net Engine • {deal.status === 'Sold' ? 'FINAL REALIZED' : 'FORECASTED'}</p>
           </div>
         </div>
      </div>
      ...
      <div className="p-8">
        ...
        <div className="text-center mb-10">
           <p className="text-sm font-semibold text-gray-400 tracking-[0.2em] mb-2 uppercase">{isBrrrr ? 'Capital Pulled Out' : 'Total Net Profit'}</p>
           ...
           <div className="mt-4 flex items-center justify-center space-x-6">
              <div className="flex items-center text-emerald-200">
                 ...
                 <span className="text-lg font-mono">{roi.toFixed(2)}% <span className="text-xs text-gray-500 ml-1">ROI</span></span>
              </div>
              <div className="flex items-center text-emerald-200">
                 <CalendarDays className="w-4 h-4 mr-1 opacity-50" />
                 <span className="text-lg font-mono">{annualizedIrr.toFixed(2)}% <span className="text-xs text-gray-500 ml-1">IRR (Ann.)</span></span>
              </div>
              <div className="flex items-center text-emerald-200">
                 <span className="text-lg font-mono">{Math.round(holdDays)} <span className="text-xs text-gray-500 ml-1">Hold Days</span></span>
              </div>
           </div>
        </div>

'use client';

import React from 'react';
import { motion } from 'framer-motion';

export default function TrustBanner() {
  return (
    <div className="bg-bg-surface py-12 border-b border-border-accent/50 overflow-hidden">
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        <p className="text-center text-xs font-bold text-text-secondary uppercase tracking-[0.2em] mb-10">
          Trusted by the World&apos;s Most Sophisticated Operators
        </p>

        <div className="flex flex-wrap items-center justify-center gap-x-12 gap-y-10 opacity-40 grayscale transition-all hover:opacity-70">
           {/* Placeholder for Logos - In production, these would be SVG components or Images */}
           <div className="flex items-center space-x-2 font-black text-xl tracking-tight text-text-primary">
              <span className="w-5 h-5 bg-gray-900 rounded-sm" />
              <span>GOOGLE</span>
           </div>
           
           <div className="flex items-center space-x-1 font-black text-xl tracking-tighter text-text-primary italic">
              <span className="text-2xl font-black">S</span>
              <span>STRIPE</span>
           </div>

           <div className="flex flex-col items-center">
              <span className="text-xs font-bold leading-none tracking-widest text-text-primary mb-0.5">COMPASS</span>
              <div className="w-12 h-0.5 bg-gray-900" />
           </div>

           <div className="flex items-center space-x-2 group">
              <div className="w-8 h-8 rounded-full border-4 border-gray-900 flex items-center justify-center font-bold text-xs">CB</div>
              <span className="text-sm font-black tracking-tight text-text-primary">COLDWELL BANKER</span>
           </div>

           <div className="flex flex-col items-end">
              <span className="text-lg font-black tracking-tighter text-text-primary">ZILLOW</span>
              <span className="text-xs font-extrabold text-text-primary uppercase tracking-[0.3em] -mt-1">PREMIER AGENT</span>
           </div>
        </div>
      </div>
      
      {/* Subtle fade line */}
      <div className="w-full h-px bg-gradient-to-r from-transparent via-gray-200 to-transparent mt-12 opacity-50" />
    </div>
  );
}

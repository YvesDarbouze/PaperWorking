...
import { generateTaxDocument, TaxFormType } from '@/lib/tax/document-generator';
import { generateYearEndWorkflow } from '@/lib/tax/workflows';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const workflow = generateYearEndWorkflow(sampleDatapoints, taxYear);
    const formsToGenerate: TaxFormType[] = ['Schedule-E', 'Form-4562', 'Schedule-D'];

    const generatedDocs = await Promise.all(
      formsToGenerate.map(form => generateTaxDocument(sampleDatapoints, form, taxYear))
    );
    ...
    return NextResponse.json({ error: 'Failed to generate tax package', details: errMsg }, { status: 500 });
  }
}

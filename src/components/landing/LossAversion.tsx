      ...
      <div className="mx-auto max-w-5xl px-6 lg:px-8">
        ...
        <div className="border border-phase-1 bg-dashboard overflow-hidden">
          ...
          {comparisons.map((row, i) => (
            <div key={row.category} className={`grid grid-cols-3 ${i < comparisons.length - 1 ? 'border-b border-phase-1' : ''}`}>
              {/* Category */}
              <div className="p-4 sm:p-5 bg-bg-surface">
                <span className="text-xs font-bold text-text-primary">{row.category}</span>
              </div>

              {/* Manual — pain */}
              <div className="p-4 sm:p-5 bg-bg-surface border-l border-phase-1">
              ...
              {/* PaperWorking — gain */}
              <div className="p-4 sm:p-5 bg-dashboard border-l border-phase-1">
                <span className="text-sm font-bold text-text-primary">{row.pw.label}</span>
        ...
        <div className="mt-8 text-center">
          <p className="text-sm text-phase-3">
            ...
            <span className="font-bold text-text-primary">$57,600 saved annually</span>{' '}

    ...
    <section className="rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden">
      ...
      >
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-lg bg-pw-subtle/30 flex items-center justify-center">
            <HandCoins className="w-4.5 h-4.5 text-gray-600" />
          </div>
          <div className="text-left">
            <h2 className="text-base font-semibold text-gray-900 tracking-tight">Pledge Tracker</h2>
      ...
      {expanded && (
        <div className="px-6 pb-6 space-y-4">
          ...
          {pledges.length > 0 && (
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              <div className="bg-gray-50 rounded-lg px-4 py-3">
                <p className="text-xs font-bold text-gray-400 uppercase tracking-widest">Total Pledged</p>
                <p className="text-lg font-semibold text-gray-900 mt-0.5">${stats.totalPledged.toLocaleString()}</p>
              </div>
                ...
                <thead>
                  <tr className="border-b border-gray-100">
                    {['Investor', 'Amount', 'Equity', 'Deadline', 'Status', 'Actions'].map((h) => (
                      <th key={h} className="text-left text-xs font-bold text-gray-400 uppercase tracking-widest py-2 px-2">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {pledges.map((pledge) => {
                    ...
                    return (
                      <tr key={pledge.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition group">
                        <td className="py-3 px-2">
                          <div>
                            <p className="font-medium text-gray-900">{pledge.investorName}</p>
                            <p className="text-xs text-gray-400">{pledge.investorEmail}</p>
                          </div>
                        </td>
                        <td className="py-3 px-2 font-semibold text-gray-900">${pledge.pledgeAmount.toLocaleString()}</td>
                        <td className="py-3 px-2 text-gray-600">{pledge.pledgeEquity > 0 ? `${pledge.pledgeEquity}%` : '—'}</td>
                        <td className="py-3 px-2">
                          <div className="flex items-center gap-1">
                            {urgent && <AlertTriangle className="w-3 h-3 text-red-500" />}
                            <span className={`text-xs ${urgent ? 'text-red-500 font-bold' : 'text-gray-500'}`}>
                              {daysLeft}d left
                            </span>
                          </div>
                        </td>
          ...
          {/* Add Pledge Form */}
          {showAddForm ? (
            <div className="border border-gray-200 rounded-xl p-5 space-y-3 bg-gray-50/40">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-semibold text-gray-900">Record Investor Pledge</h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-1">Investor Name</label>
                  <input
                    value={formName}
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full border border-gray-200 rounded-lg pl-9 pr-3 py-2 text-sm focus:border-gray-400 focus:ring-1 focus:ring-gray-400 transition"
                    />
                  </div>
                </div>
                <div>
                  <label className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-1">Pledge Amount</label>
                  <div className="relative">
                ...
                <div>
                  <label className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-1">Equity %</label>
                  <div className="relative">

...
import { GeneratedReport } from './report-builder';
...
export async function exportReportPdf(report: GeneratedReport): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    try {
      ...
      doc.fillColor('#10b981').fontSize(20).font('Helvetica-Bold').text('PAPERWORKING PORTFOLIO REPORT', 40, 20);
      doc.fillColor('#ffffff').fontSize(11).font('Helvetica').text(`Executive Performance Report (${report.type.toUpperCase()})`, 40, 46);

      doc.moveDown(3);

      // Executive Summary
      doc.fillColor('#0f172a').fontSize(14).font('Helvetica-Bold').text('Executive Summary', 40, 100);
      doc.fontSize(10).font('Helvetica').fillColor('#334155').text(report.executiveSummary, 40, 120, { width: 500 });

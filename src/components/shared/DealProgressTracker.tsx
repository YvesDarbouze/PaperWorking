               ...
               return (
                 <React.Fragment key={phase}>
                   <div className="flex flex-col items-center group relative cursor-default">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center border-2 transition-all duration-300 ${isActive ? 'bg-[#454955] border-[#20B2AA] text-white shadow-[0_0_15px_rgba(32,178,170,0.5)] scale-110' : isCompleted ? 'bg-emerald-500 border-emerald-400 text-white' : 'bg-slate-800 border-slate-700 text-text-secondary'}`}>
                         {isCompleted ? <CheckCircle className="w-5 h-5"/> : isActive ? <MapPin className="w-4 h-4" /> : <span className="text-xs font-bold">{idx + 1}</span>}
                      </div>
                      <span className={`text-xs mt-1.5 font-bold uppercase tracking-wider ${isActive ? 'text-[#20B2AA]' : isCompleted ? 'text-emerald-400' : 'text-text-secondary'}`}>
                         {phase}
                      </span>
                   </div>

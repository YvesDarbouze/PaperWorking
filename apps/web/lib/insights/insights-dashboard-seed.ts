...
export interface InvestorKpiCard {
  ...
  prior: number | null;
  /** Provenance of the metric data: computed by engine vs illustrative demo data */
...
export const INVESTOR_KPI_SECTIONS: InvestorKpiSection[] = [
  {
    ...
    metrics: [
      ...
      },
      {
        ...
        unit: 'percent',
        higherIsBetter: true,
        formula: '(NOI ÷ Total Cost Basis) × 100',
        description: 'Unlevered return on all-in capital basis.',
        prior: 8.1,
        dataProvenance: 'illustrative_demo',
      },
      {
...
export interface ComparisonPoint {
  ...
  /** Provenance of the comparison point: computed by engine vs illustrative demo data */
  dataProvenance?: 'computed' | 'illustrative_demo';
}

/**
 * Computes comparison points dynamically from seed deal underwriting snapshots
 * through the canonical financial engine outputs.
 */
export function computeSeedComparisonPoints(): ComparisonPoint[] {
  const targetIds = ['deal-1', 'deal-2', 'deal-3'];
  return targetIds.map((id) => {
    ...
    if (!project) {
      return {
        projectId: id,
        projectName: id,
        metrics: { cap_rate: 0, cash_on_cash: 0, dscr: 0, ltv: 0, oer: 0, grm: 0 },
        dataProvenance: 'illustrative_demo',
      };
    }
    ...
    const capRate = outputs?.capRateOnCost ?? 0;
    const cashOnCash = outputs?.cashOnCashReturnPct ?? 0;
    const dscr = outputs?.dscr ?? 0;
    const ltv = outputs?.ltvPct ?? 0;
    const oer = inputs?.operatingExpenseRatioPct ?? 0;
...
export const COMPARISON_POINTS: ComparisonPoint[] = computeSeedComparisonPoints();

export const COMPARE_METRIC_OPTIONS = [
  { id: 'cap_rate', name: 'Cap Rate on Cost', suffix: '%' },
...
export const TREND_METRIC_OPTIONS = [
  { id: 'noi', name: 'Net Operating Income', color: '#00DD94', unit: 'currency' as const },
  { id: 'cash_flow', name: 'Net Cash Flow', color: '#9E9DA0', unit: 'currency' as const },
  { id: 'occupancy', name: 'Occupancy Rate', color: '#F06543', unit: 'percent' as const },
] as const;
...
export const INSIGHTS_TAB_CATEGORIES = [
  ...
  { id: 'portfolio', name: 'Asset and Portfolio Management', icon: 'layers', match: 'Portfolio Metrics' },
  { id: 'marketing', name: 'Marketing & Sales', icon: 'group', match: 'Syndication Metrics' },
  { id: 'compliance', name: 'Risk Management and Compliance Metrics', icon: 'verified_user', match: 'Portfolio Metrics' },
] as const;

...
function classifyOccupancy(rate: number): {
  ...
  if (rate >= 88) return { grade: 'caution', label: 'Below National Average', description: 'U.S. average is ~90% — review pricing, marketing, and property condition', color: '#A5A5A5', bgColor: 'rgba(165,165,165,0.08)', borderColor: 'rgba(165,165,165,0.2)' };
...
function OccTooltip({ active, payload }: any) {
  ...
  return (
    <div className="rounded-lg px-3 py-2 shadow-lg text-xs" style={{ background: 'var(--bg-surface)', border: '1px solid var(--border-ui)' }}>
      ...
      <p className="tabular-nums" style={{ color: '#7F7F7F' }}>Occupancy: {fmtPct(d.occupancy)}</p>
...
export default function OccupancyDeepDive({ projects: propProjects }: Props) {
  ...
  const gaugeSegments = [
    ...
    { min: 88, max: 93, label: '88-93%', color: '#A5A5A5', desc: 'Below Avg' },
    { min: 93, max: 97, label: '93-97%', color: '#7F7F7F', desc: 'Healthy' },
    { min: 97, max: 100, label: '97%+', color: '#595959', desc: 'Full' },
  ];
  ...
  return (
    <div className="w-full space-y-6">
        ...
        <div className="bg-bg-surface border border-border-accent rounded-xl p-5 flex flex-col" style={{ minHeight: '280px' }}>
          ...
          <div className="flex-1 min-h-0 w-full relative">
            <div className="absolute inset-0">
              <OccupancyChart data={analysis.breakdowns} height="100%" />
            </div>
          </div>
        </div>
      )}
      ...
      <div className="px-4 py-3 rounded-lg text-[11px] leading-relaxed" style={{ background: 'rgba(127,127,127,0.05)', border: '1px solid rgba(127,127,127,0.15)', color: 'var(--text-secondary)' }}>
        ...
        <strong style={{ color: '#A5A5A5' }}>The 100% trap:</strong>{' '}

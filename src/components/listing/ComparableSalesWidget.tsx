...
const ComparableSalesWidget: React.FC<ComparableSalesWidgetProps> = ({ 
  ...
  return (
    <div className="bg-bg-surface border border-border-accent">
      <div className="bg-pw-black px-10 py-8 border-b border-border-accent flex justify-between items-center">
        <div>
        ...
        {comps.map((comp) => (
          <div key={comp.id} className="p-10 hover:bg-bg-primary transition-all flex justify-between items-center group cursor-crosshair">
            <div className="flex items-center gap-8">
              ...
              <div>
                <p className="text-base font-black text-text-primary uppercase tracking-tighter mb-2 group-hover:text-pw-accent transition-colors">{comp.address}</p>
                <div className="flex items-center gap-8 text-xs font-black text-text-secondary uppercase tracking-[0.2em] font-mono">
            ...
            <div className="text-right flex items-center gap-16">
              <div className="hidden lg:block">
                <p className="text-xs text-text-secondary font-black uppercase mb-2 tracking-[0.3em]">SPECIFICATION</p>
                <p className="text-sm font-black text-text-primary uppercase tracking-tighter font-mono">
                  {comp.beds}B / {comp.baths}BA / {comp.sqft} SQFT
                </p>
              </div>
              <div>
                <p className="text-xs text-text-secondary font-black uppercase mb-2 tracking-[0.3em]">SETTLEMENT</p>
                <p className="text-2xl font-black text-text-primary tracking-tighter font-mono">
      ...
      <div className="p-10 bg-bg-primary border-t border-border-accent flex justify-center">
        <button className="flex items-center gap-4 text-xs font-black text-text-secondary hover:text-pw-accent transition-all uppercase tracking-[0.4em] border border-transparent hover:border-pw-accent px-8 py-4">

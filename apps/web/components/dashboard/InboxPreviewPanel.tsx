...
export default function InboxPreviewPanel() {
  ...
  const [allThreads, setAllThreads] = useState<InboxThread[]>([]);
  ...
  const threads = useMemo(() => {
    return allThreads.filter((thread) => (tab === 'all' ? true : thread.tab === tab));
  }, [tab, allThreads]);
  ...
  return (
    <div className="w-full min-w-0 space-y-6 px-4 py-5 sm:px-5 sm:py-6 lg:px-6 lg:py-7 xl:px-8">
      <DashboardPageHeader
        ...
        actions={
          <DashboardPrimaryButton href="/dashboard/inbox" icon="edit">
            Compose
          </DashboardPrimaryButton>
        }
      />
        ...
        <>
          ...
          <div className="grid gap-4 lg:grid-cols-[1fr_1.1fr]">
            <div className="overflow-hidden rounded-2xl border border-white/10 bg-[#121014]/90">
              {threads.length === 0 ? (
                <p className="p-8 text-sm text-white/45">No messages yet</p>
              ) : (
                threads.map((thread) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <p
                        className={`text-sm ${isUnread ? 'font-semibold text-white' : 'text-white/75'}`}
                      >
                        {thread.subject}
                      </p>

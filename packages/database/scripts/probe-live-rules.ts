...
export async function runLiveRulesProbes(config?: {
...
}): Promise<{ passed: boolean; results: LiveProbeResult[] }> {
  const projectId =
    config?.projectId ||
  ...
  // Connect to local emulators if emulator environment is active
...
export function formatProbeReport(report: { passed: boolean; results: LiveProbeResult[] }): string {
  const lines: string[] = [];
  lines.push('================================================================================');

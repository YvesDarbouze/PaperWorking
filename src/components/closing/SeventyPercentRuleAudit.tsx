...
export default function SeventyPercentRuleAudit() {
  ...
  if (!audit || audit.arv === 0) {
    return (
      <div className="bg-bg-primary rounded-xl border border-border-accent p-6 flex items-center gap-3">
        <Info className="w-5 h-5 text-text-secondary flex-shrink-0" />
        <div>
          <p className="text-sm text-text-secondary font-medium">70% Rule Audit Unavailable</p>
          <p className="text-xs text-text-secondary mt-0.5">Set an estimated ARV in deal financials to enable this audit.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={`rounded-xl border-2 ${cfg.border} overflow-hidden`}>
      ...
      <div className={`${cfg.bg} p-6`}>
        ...
        {/* Formula display */}
        <div className="mt-4 p-4 bg-bg-surface/70 backdrop-blur rounded-lg border border-border-accent">
          <p className="text-xs uppercase tracking-widest text-text-secondary mb-2">70% Rule Formula</p>
          <div className="flex items-center gap-2 flex-wrap text-sm font-mono">
            <span className="text-text-secondary">MAO =</span>
            <span className="text-text-primary">(${audit.arv.toLocaleString()} × 0.70)</span>
            <span className="text-text-secondary">−</span>
            <span className="text-text-primary">${audit.totalRepairs.toLocaleString()}</span>
            <span className="text-text-secondary">=</span>
            <span className={`font-semibold ${audit.maxAllowable >= 0 ? 'text-text-primary' : 'text-red-600'}`}>
        ...
        <div className="mt-4">
          ...
          <div className="h-3 bg-bg-surface/60 rounded-full overflow-hidden relative">
            {/* 100% marker */}
          ...
          <div className="flex justify-between text-xs mt-1.5">
            <span className="font-mono text-text-primary">${audit.totalCapitalized.toLocaleString()}</span>
      ...
      <button
        ...
        className="w-full flex items-center justify-between px-6 py-3 bg-bg-surface hover:bg-bg-primary transition border-t border-border-accent"
      >
        <span className="text-xs font-medium text-text-secondary">Cost Breakdown</span>
      ...
      {showDetails && (
        <div className="px-6 py-4 bg-bg-surface space-y-2">
          {audit.breakdown.map((b, i) => (
            <div key={i} className="flex items-center justify-between py-1.5 border-b border-gray-50 last:border-0">
              <span className="text-sm text-text-secondary">{b.label}</span>
              <span className="text-sm font-mono text-text-primary">${b.amount.toLocaleString()}</span>
            </div>
          ))}
          <div className="flex items-center justify-between py-2 border-t-2 border-border-accent mt-2">
            <span className="text-sm font-semibold text-text-primary">Total Capitalized</span>
            <span className={`text-sm font-mono font-bold ${audit.isOverBudget ? 'text-red-600' : 'text-text-primary'}`}>
